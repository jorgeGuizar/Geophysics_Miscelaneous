...
fig, ax = plt.subplots()
im = ax.imshow(vx, cmap='RdBu', extent=(0, nx*dx, 0, nz*dz), animated=True)
ax.set_title('Propagación de onda elástica')
plt.show()

def update(n):
    global vx, vz, txx, tzz, txz
    ...
    #vx[1:-1, 1:-1] += dt * (txx[1:-1, 1:-1] - txx[:-2, 1:-1]) / (rho * dx) + dt * (txz[1:-1, 1:-1] - txz[1:-1, :-2]) / (rho * dz)
    #vz[1:-1, 1:-1] += dt * (txz[1:-1, 1:-1] - txz[:-2, 1:-1]) / (rho * dx) + dt *