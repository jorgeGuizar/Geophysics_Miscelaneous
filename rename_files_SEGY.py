...
def rename_files_2(file_list,extension,a):
    len_ext=len(extension)
    for i in range(len(file_list)):
        print("File {} has been renamed to {}".format(file_list[i],file_list[i][:-a]+file_list[i][-len_ext:]))
        #os.rename(file_list[i],file_list[i][:-15]+".sgy"+file_list[i][-4:])
        os.rename(file_list[i],file_list[i][:-a]+file_list[i][-len_ext:])
    return
