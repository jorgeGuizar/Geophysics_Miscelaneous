...
file_name=r'C:\Users\LEGA\Documents\Geofisica\MB\etkal\ETKAL_REV2.txt'
...
window_size=2000; # good test were run from 600
ZZ=[]
for ping in pinger:
...
# # Plot the surfac
# #yi=np.unique(Y)
xi = np.linspace(min(Xf), max(Xf), 3000)
yi = np.linspace(min(Yf), max(Yf), 3000)
